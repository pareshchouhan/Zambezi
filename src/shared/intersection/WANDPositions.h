#ifndef WAND_POSITIONS_H_GUARD
#define WAND_POSITIONS_H_GUARD
...
Candidate** wandPositions(PostingsPool* pool, long* headPointers, int* df, float* UB, int len,
  ...
  Candidate** set = malloc((elements->index + 1) * sizeof(Candidate**));
  for(i = 1; i < elements->index + 1; i++) {
    set[i - 1] = elements->candidate[i];
  }
  set[i - 1] = NULL;
  destroyHeapWithPositions(elements, 0);
  return set;
}
